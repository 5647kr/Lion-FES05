class VendingMachineEvent {
  ...
  bindEvent() {
    /**
      * 1. 입금 버튼 기능
      * 1.1 입금 버튼을 누르면 소지금 === 소지금 - 입금액
      * 1.2 잔액 === 기존 잔액 + 입금액
      * 1.3 입금액이 소지금보다 많으면 경고창 출력
      * 1.4 입금액이 정상적으로 입금되면 입금창은 초기화
    */ 
    this.btnPut.addEventListener("click", () => {
      ...
      if(inputCost) {
        ...
        // 입금액 초기화
        this.inputCostEl.value = null; // "" 써도 상관없다
      }
    });
    
    /**
      * 2. 거스름돈 반환 버튼
      * 2.1 반환버튼을 누르면 소지금 === 잔액 + 소지금
      * 2.2 반환버튼을 누르면 잔액창이 초기화
    */
    ...
    /**
      * 3. 자판기 장바구니 채우기
      * 3.1 아이템을 누르면 잔액 === 잔액 - 아이템 가격
      * 3.2 아이템 가격이 잔액보다 크다면 경고메세지를 준다.
      * 3.3 아이템이 장바구니에 들어간다.
      * 3.4 아이템의 count가 -1이 된다. 아이템의 count가 0이 되면 품절 표시
    */
    ...
    this.btnsCola.forEach((item) => {
      item.addEventListener("click", (event) => {
        ...
        if(balanceVal >= targetElPrice) {
          ...
          for(const item of stagedListitem) {
            ...
            if(targetEl.dataset.item === item.dataset.item) {
              ...
              item.querySelector("strong").firstChild.textContent = parseInt(item.querySelector("strong").firstChild.textContent
              ) + 1;
        ...
        /**
      * 4. 획득 버튼 기능
      * 4.1 장바구니에 있는 음료수 목록이 획득한 음료 목록으로 이동한다.
      * 4.2 획득한 음료의 모든 금액을 합하여 총 금액으로 업데이트한다
    */
    ...
    this.btnGet.addEventListener("click", () => {
      ...
      this.getList.querySelectorAll("li").forEach((itemGet) => {
        // this.txtTotal.textContent = parseInt(this.txtTotal.textContent) + parseInt(itemGet.dataset.price) * parseInt(itemGet.querySelector("strong").textContent.firstChild)
